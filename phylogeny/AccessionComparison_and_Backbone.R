...
treeZ <- read.tree("~/salp_ecomorphology/phylogeny/ML/GUC-Mm+N+Sanger_MUSCLE_ML/GUC-Mm+N+Sanger_MUSCLE.afa.treefile") #Load ML tree since RevBayes one has polytomies
...
treeY <- reroot(treeX, 103) #set true root
treeU <- treeY %>% chronos() #make tree ultrametric
write.tree(treeU, "~/salp_ecomorphology/phylogeny/RevBayes/GUC-Mm+N+Sanger_MUSCLE_ML_rooted_multiD_ultra.tree")
